    ...
    game.settings.settings.forEach((setting) => {
        if (setting.module === mod.data.name) {
            ...
            if (setting.config && setting.type !== "object") {
                ...
                const ignore = ["cookie", "password", "secret", "token", "bearer", "email"].some((badKey) =>
                    trimmedKey.includes(badKey)
                );
                if (!ignore) {
...
class BugReportForm extends FormApplication {
    constructor(app, { selectedModule, title = "", description = "" }) {
        super(app);
        this.endpoint =
            "https://foundryvttbugreporter.azurewebsites.net/api/ReportBugFunction?code=VCvrWib1lha2nf9Pza7fOaThNTksbmHdEjVhIudCHwXg3zyg4vPprg==";
        ...
        this.useBugReporter =
            (this.module.data.flags?.allowBugReporter || this.module.data.allowBugReporter) &&
            (this.github || this.gitlab);

        if (this.module.data.allowBugReporter) {
            console.warn(
                `Root level allowBugReporter fields are deprecated as of BugReporter 1.3.0 and will be removed in a future release. The module ${this.module.data.name} should update its manifest to place allowBugReporter in the flags object.`
            );
        }
    ...
    async _updateObject(ev, formData) {
        ...
        const {
            formFields: { apiDetails, bugTitle, bugDescription, issuer, label, sendActiveModules, sendModSettings },
        ...
        if (this.gitlab) {
            bugsUrl =
                bugsUrl +
                `?title=${encodeURIComponent(bugTitle)}&description=${encodeURIComponent(
        ...
        await fetch(this.endpoint, {
            ...
            .then(async (res) => {
                ...
                if (res.status == 201) {
                    ...
                    ui.notifications.error(game.i18n.localize("BUG.error"));
                    console.error(
                        "Bug Reporter encountered the following problem submitting your issue. Oh the irony...",
                        res
                    );
                }
            })
    ...
    async search(event) {
        ...
        if (this.github) {
            this.foundIssues = message.items.map(({ html_url, state, created_at, title }) => ({
                html_url,
                state,
            ...
            this.foundIssues = message.map(({ web_url, state, created_at, title }) => ({
                html_url: web_url,
                state,
                openedLabel: new Date(created_at).toLocaleDateString(),
                title,
     ...
     */
    async checkVer(mod) {
        fetch("https://forge-vtt.com/api/bazaar/manifest/" + mod.data.name + "?coreVersion=" + game.data.version).then(
            (res) => {
...
function getModuleSelection() {
    return new Promise((resolve, reject) => {
        ...
        const moduleOptions = [...game.modules.values(), game.system]
            .filter((mod) => (mod.active || mod.template) && !!mod.data.bugs)
...
Hooks.once("init", () => {
    Hooks.on("renderSidebarTab", async (app, html) => {
        ...
        if (app.options.id == "settings") {
            let button = $(
                `<button class='bug-report'><i class="fas fa-bug"></i> ${game.i18n.localize(
                    "BUG.bugButton.label"
                )}</button>`
            );
